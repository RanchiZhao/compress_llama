...
def add_compress_args(parser: argparse.ArgumentParser):
    group = parser.add_argument_group("compress", "compression configurations")
    group.add_argument("--cook-config", type=str, help="path to bmcook configuration file")
    group.add_argument("--save-opt-name", type=str, help="name of optimizer state")
    group.add_argument("--log-file", type=str, help="file for recording compression process")
    group.add_argument("--cook-save", type=str, help="Output directory to save compressed checkpoints to.")
    group.add_argument("--cook-save-name", type=str, help="Output filename to save compressed checkpoints to.")
    group.add_argument("--cook-mask-save", type=str, help="Output filename to save pruning mask to.")
    group.add_argument("--cook-save-iters", type=int, default=50, help="number of iterations between compressed checkpoints saves.")
    group.add_argument("--cook-save-mask-iters", type=int, default=20, help="number of iterations between pruning mask saves.")
    group.add_argument("--cook-save-mode", type=str, help="save mode of compressed checkpoints. choose from 'quant' and 'prune'.")
    group.add_argument("--load-teacher", type=str, help="checkpoint path of teacher model.")
    group.add_argument("--teacher-config", type=str, help="config path of techer config.")
    return parser


def get_args(pretrain: bool = False, finetune: bool = False, compress: bool = False):
    ...
    if compress:
        parser = add_compress_args(parser)
