...
def convert_data_to_id(tokenizer: LlamaTokenizer, data: Any):
    input_ids = tokenizer.encode(data['input'])[1:]
    output_ids = tokenizer.encode(data['output'])[1:]
    ids = [tokenizer.bos_token_id] + input_ids + output_ids + [tokenizer.eos_token_id]
...
class _MixedDatasetBatchPacker:
    ...
    def apply_transform(
        ...
        def _expand_mapping(
            data: LlamaInputType, stars: List[str], path: List[str], target: List[str]
        ):
            if len(path) == 0:
                num_stars = 0
                for it in target:
    ...
    def pack_batch(self, force: bool = False) -> LlamaBatch:
        # pack batch
        ...
        for i in range(batch_size):
            all_task_names.add(self._task_ids[i])
